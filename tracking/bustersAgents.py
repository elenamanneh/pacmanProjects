    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        mostLikelyPositions = [distribution.argMax() for distribution in livingGhostPositionDistributions if distribution.total() > 0]

        closestGhostDist = float("inf")
        actionTowardsClosestGhost = Directions.STOP
        for action in legal:
            ...
            for ghostPosition in mostLikelyPositions:
                ...
                if distanceToGhost < closestGhostDist:
                    closestGhostDist = distanceToGhost
                    actionTowardsClosestGhost = action

        return actionTowardsClosestGhost
        "*** END YOUR CODE HERE ***"